    ...
    const handleSubmit = async (e) => {
        ...
        if (inputMessage.trim() !== "") {
            try {
                ...
                const response = await axios.post('/api/chatgpt/chatbot', {
                    messages: updatedMessages
                })
                const sereverResponse = response.data;

                //Add the server response to the messages array
                const updatedMessages2 = [...updatedMessages, {
                    role: "assistant",
                    content: sereverResponse.data.choices[0].message.content
                }];
                ...
                // Update jResult with the updated messages array
            ...
            <div className='d-flex flex-column chat-page'>
                ...
                <div id='chatContainer' className='flex-fill overflow-auto'>
                    {messages.map((message, index) => message.role !== "system" && (
                        <div key={index}
                             className={`${message.role === 'user' ? 'alert alert-info' : 'alert alert-success'}`}>{message.content}</div>
                    ))}
